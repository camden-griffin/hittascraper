...
// Puppeteer's browser-command channel timeout. The default (120s) can fire on
// a long-lived headless Chrome as a "ProtocolError: ... timed out" unhandled
// rejection, forcing a restart. Raise it and recycle the browser often (short
// sessions) so the stall never accumulates. 0 = leave scraper.js default.
const PROTOCOL_TIMEOUT_MS = intArg("protocolTimeoutMs", 300_000);
  ...
  return new Promise((resolve) => {
    const env = {
      ...
      ...(PROTOCOL_TIMEOUT_MS > 0
        ? { PROTOCOL_TIMEOUT_MS: String(PROTOCOL_TIMEOUT_MS) }
        : {}),
...
(async () => {
  ...
  console.log(
    `Per-org delay:    ${MIN_DELAY_MS}–${MAX_DELAY_MS} ms (random/session)`,
  );
  ...
  console.log(`Protocol timeout: ${PROTOCOL_TIMEOUT_MS}ms`);
  ...
  let nextLongRestAt =
    LONG_REST_EVERY > 0
      ? randInt(Math.max(1, LONG_REST_EVERY - 2), LONG_REST_EVERY + 2)
      : 0;
  ...
  while (!stopping) {
    ...
    let restMs;
    const isLongRest = LONG_REST_EVERY > 0 && session >= nextLongRestAt;
    if (blocked) {
  ...
  console.log(
    `Resume any time:  node src/scrape_full_throttled.js --input ${INPUT}`,
  );
  console.log("====================================");
})().catch((e) => {
